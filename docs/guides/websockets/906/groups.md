...
`socket_type` — это поле команды `init_socket`. В сборке 906 `libMain.main_socket` копирует его значение в тег соединения с тем же именем. Поэтому `socket_type` можно использовать для группировки через API тегов, хотя оно передаётся отдельно от массива `socket_tags`.
