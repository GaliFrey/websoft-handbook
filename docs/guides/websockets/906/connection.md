# WebSocket 906, 1132, 1333 и 1525: подготовка и подключение
...
значением должен быть один JSON-объект. Сборки 1132, 1333 и 1525 также принимают
...
На стенде 1525 после такой ошибки соединение осталось активным и ответило `$ok`
на следующий heartbeat. Ошибка отдельного сообщения сама по себе не закрывает
WebSocket.

### Проверка связи

В сборках 906, 1132, 1333 и 1525 тестовый клиент может отправлять строку `$hbt`
...
проверка изначально выключена. На стенде это поведение проверено для 906 и 1525.
