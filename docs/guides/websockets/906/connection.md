# WebSocket 906: подготовка и подключение
...
### Проверка связи в сборке 906

В сборке 906 тестовый клиент может отправлять строку `$hbt` и отмечать ответ `$ok`. DLL перехватывает это служебное сообщение до вызова `main_ws_service`, поэтому оно не попадает в `main_socket`. Автоматическая проверка изначально выключена.
