# WebSocket 906–1515: закрытие соединений

Примеры этой главы проверены на стенде 906. В 1515 изменена обработка кодов
закрытия, поэтому фактический код при отказе или ошибке нужно проверить на
целевом стенде. Методы `Abort` и `GetWebSockets`, используемые ниже, доступны во
всех рассматриваемых сборках.
...
сборках 906, 1132, 1333 и 1515. Для обычного закрытия со стороны тестового
клиента нажмите «Отключиться». Принудительное прерывание через `Abort()`
проверено ниже только на стенде 906.

Обработчик `close(context)` одинаков в сборках 906–1515. При
...
[К обзору сборок 906–1515](index.md).
