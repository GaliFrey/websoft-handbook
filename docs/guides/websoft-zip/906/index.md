# Websoft.Zip — сборка 906

Руководство относится к WebSoft HCM Server `2023.2.906` и
Websoft.Zip `1.24.5.14`. Примеры выполняются в серверном агенте.

!!! danger "Встроенная защита неполна"
    DLL блокирует проверенные `../../two.txt`, `sub/../../mixed.txt` и
    абсолютный путь, но извлекает `../one.txt` за пределы каталога назначения.
    Распаковывайте только доверенные архивы.
...
### Граница защиты путей

В `Extract()` и `ExtractFiles()` добавлены проверки абсолютных и опасных
путей. На стенде получены следующие результаты:

| Имя записи | Результат | Файл создан |
| --- | ---: | ---: |
| `safe.txt` | `1` | да |
| `../one.txt` | `1` | да, вне каталога назначения |
| `../../two.txt` | `0`, `Invalid path` | нет |
| `sub/../../mixed.txt` | `0`, `Invalid path` | нет |
| абсолютный путь | `0`, `Prohibited rooted path` | нет |
...
Ошибки DLL этой сборки записываются в штатный журнал `zip_`.
